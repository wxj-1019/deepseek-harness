...
| [`mcp/`](mcp/README.md) | MCP（模型上下文协议）桥接：服务器组装管理器 + 将外部服务器工具注册进来的客户端桥接 | 产品：稳定 API |
...
| [`runtime-diagnostics/`](runtime-diagnostics/README.md) | 运行时诊断：不变量注册、健康断言和伴随插件所有权 | 产品：稳定 API |
