...
Each query runs one semantic operation at a cursor position, over a whole file, or across the workspace; results are normalized locations, hover content, symbol outlines, diagnostic rows, call rows, or edit plans — never raw protocol payloads.
...
| `documentSymbol` | The file's symbol outline with containers and ranges |
| `workspaceSymbol` | Workspace-wide symbol matches for a query string |
| `diagnostics` | Pulled (or push-fallback) diagnostics for one file |
| `rename` | A normalized per-file edit plan for the new identifier |
| `formatting` | A whole-document edit plan |
| `incomingCalls` / `outgoingCalls` | Call rows with call-site spans behind a prepared symbol |

The seam exposes this closed set of semantic operations and no generic JSON-RPC escape hatch, so no protocol payload or unreviewed command/mutation reaches a provider through `ctx.lsp`. `findReferences` always includes declarations, so impact analysis never misses the defining site. Positions are zero-based UTF-16 on the wire; the model-facing tool accepts one-based cursor coordinates and converts them.
...
- **Closed vocabulary.** The operation union is closed — adding an operation is a compile-enforced change across the seam, providers, and the tool. There is no JSON-RPC escape hatch, and every request field is required, so there is no `resolve()` step.
...
- **No applied mutations at the seam** — rename and formatting return edit PLANS the model applies with its own file-edit tools (the tool layer owns the opt-in host-apply path), and the host rejects `workspace/applyEdit`. Code actions remain deferred (preview, permission, and write-policy integration).
