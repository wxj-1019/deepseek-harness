...
`ctx.lsp` 的**通用 stdio 语言服务器后端**。一个插件实例接受一张命名服务器表，并逐配置项注册一个隔离的提供方。它通过 `ctx.fs` 读取，并通过 `ctx.subprocess` 启动，因此服务器与源文件始终位于已挂载的执行世界中。主机本身保持通用：内置一份默认开启的语言目录种子（见配置），但它不是安装器——需要特定服务器的部署应显式配置命令与映射，预设应放在 `cordis.yml` overlay 中。
...
`catalog`（默认 `true`）会为没有显式 `servers` 条目的 id 从内置语言目录播种 TypeScript 与 Python 服务器。可执行文件在主机上缺失的种子会带警告跳过；若所有显式条目也全部失败则加载仍然响亮失败，catalog 关闭且表为空会在加载时被拒绝。当至少一个提供方解析成功时，插件会注册一个有界的 `lsp:language-catalog` 提示段，列出每个服务器的扩展名与语言，使模型只对可服务的文件发起 `lsp` 查询。
...
- **诊断为拉取式** —— 宿主按请求查询 `textDocument/diagnostic`；推送式 `publishDiagnostics` 订阅不做桥接。不支持拉取诊断的服务器返回 `LSP_UNSUPPORTED_OPERATION`。
- **重命名返回计划** —— 归一化后的按文件编辑列表返回给模型，由其用文件编辑工具自行应用；宿主不代为落盘。
