...
| `catalog` | `true` | Seed a TypeScript and a Python server from the built-in language catalog for ids without an explicit `servers` entry |
| `publishGraceMs` | `250` | Bounded wait for a `publishDiagnostics` push when pulled diagnostics are unsupported |
...
- **Diagnostics pull with a bounded push fallback** — diagnostics pull from `textDocument/diagnostic` when the server advertises it; a server without pull support opens the document transiently and waits a bounded `publishGraceMs` for a `publishDiagnostics` push, returning the latest pushed set for that document.
