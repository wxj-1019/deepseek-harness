...
Loading the plugin scans `packages/client/*/src/client` for exported PascalCase components, resolves each component's props type (`<Name>Props`, then the first parameter's annotation, then an exported `Props` type), collects the sibling CSS module's `--dsw-*` references, and lifts a usage example from the component's specs or its JSDoc `@example`. Records land in the `component_library` domain; every durable write broadcasts `component-library/changed` after the domain commits. With `watch` on, a settled `.tsx` or `*.module.css` change under a package's `src/client` re-learns exactly one file, a `.tsx` removal drops its records, and a settled theme-stylesheet change re-reads the token inventory; everything else under the client tree is ignored. A `component_record` path is normalized into the repository-relative POSIX form and rejected unless it names a file under `packages/client`, and `pkg` must match the owning directory's manifest name, so model-derived ids always stay inside the scanner's id space. Model-contributed records from `component_record` are quarantined (`reviewed: false`) until approved on the panel; queries exclude them unless the `component-library` settings namespace sets `includeUnreviewed`, and the `review` face rejects scanned ids as `scanned-record`.
