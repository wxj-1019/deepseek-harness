 ...
 *
 * The plugin is one Cordis service (`ComponentLibraryService`, a Typert
 * Remote service) that owns the `component_library` domain and orchestrates
 * the scanner, watcher, tools, prompt section, and skills channel.
...
export class ComponentLibraryService extends TypertRemoteService {
   ...
   */
  @Remote('query')
  query(request: ComponentLibraryQueryRequest): Promise<ComponentLibraryQueryResult> {
    return Promise.resolve({ ok: true, value: Object.freeze({ matches: this.rankMatches(request) }) })
  }
   ...
   */
  @Remote('summary')
  summary(): Promise<ComponentLibrarySummaryResult> {
    const table = this.requireTable()
    let scanned = 0
    let pendingReview = 0
    for (const [, record] of table.entries()) {
    ...
    return Promise.resolve({ ok: true, value: Object.freeze({ total: table.size, scanned, pendingReview }) })
  }
   ...
   */
  @Remote('list')
  list(): Promise<ComponentLibraryListResult> {
    return Promise.resolve({ ok: true, value: Object.freeze({ items: this.snapshotAll() }) })
  }
   ...
   */
  @Remote('record')
  record(request: ComponentLibraryRecordRequest): Promise<ComponentLibraryRecordResult> {
