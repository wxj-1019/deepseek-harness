---
description: "dsh Web 客户端的浏览器侧桌面完成通知，基于会话列表快照流。"
...
Web 桌面通知特性所有者：提供"常规设置"里的开关行，为部署开启"任务完成时弹系统通知"，以及真正发出通知的完成监视器。信号完全来自 [`dsh-client-runtime`](../store/README.zh.md) 由 `host/session-status` 帧驱动的 `sessions.list` 快照流，本包自身不发起任何 RPC；偏好持久化在 `ui-desktop-notify` 设置命名空间。
...
监视器是快照流之上的纯客户端消费方：无 RPC、无会话事件、无宿主插件；覆盖由 e2e 场景承担。
