...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.pinned`, `sidebar.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store. The `sidebar.pinned` seat sits between the controls and the browsing region (pinned sessions ride it); an absent occupant renders nothing.
