...
`SidebarRootComponentProps` 组合布局属主份额、全局 `useSessions` 与 `useWorkspaces` hooks、已声明的 brand、`sidebar.pinned`、`sidebar.workspaces` 与 `sidebar.settings` 子槽位，以及注入的 `startSession` 与侧栏开合回调。没有插件自有 store。`sidebar.pinned` 座位位于控件与浏览区之间（置顶会话落于此）；无占用者时不渲染任何内容。
