---
description: "dsh Web 客户端的 Aqua 玻璃主题插件：mica/兼容模式、模糊与亮度旋钮、WebGL 流体或壁纸背景，以及持久的 ui-aqua 设置。"
...
渲染只做副作用：关掉总开关即可完整还原原生 UI。吸收来的引擎保留本地 tsconfig 覆盖（关闭 noUncheckedIndexedAccess）与子串选择器样式表，兄弟包改名可能需要在这里跟进。
