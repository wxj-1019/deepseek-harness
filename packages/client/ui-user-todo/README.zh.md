...
挂载存活期间，到期时刻已过的条目会触发逐条去重的桌面通知——但前提是站点已持有通知权限；观察器从不主动索权，并随挂载销亡，提醒因此活在打开的窗口里，而不是后台服务中。任何已提交的变更——自己的、其他窗口经白名单 `user-todo/changed` 推送的、以及 `connection/reset`——都会让已加载的清单重拉一次；冷清单在首次打开前保持冷态。
