...
面板在客户端派生“今日”视图：先按创建序给出全部未完成条目（从创建日起结转），再按完成时间倒序给出今天完成的条目；本地日期归桶是浏览器时钟上的纯函数，宿主不落盘任何按天簿记。行操作支持从输入框新增、切换完成戳（由宿主拥有）、行内改标题、删除、每行备注编辑器（Ctrl+Enter 提交；清空即清除）、到期 chip（点击展开本地时区的日期时间编辑器；逾期高亮，有到期者排序靠前）、由标准 `useWorkspaces` 供给的项目链接选择器，以及——链接项目后——该工作区已登记且未归档会话的会话选择器，与经 `ctx.sessions.open` 打开所链接会话的直达按钮。清除项目选择会在宿主侧级联摘掉会话链接。今日列表下方有一个可折叠区，按完成时间倒序展示更早的完成项（每行标注其本地完成日期），历史一步可达而不挤占今日视图。
