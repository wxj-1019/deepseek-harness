...
- **No Session deletion control** — sessions can be archived and unarchived (the collapsible archived block under the list lists every archived session with a restore action), but Workspace registration deletion does not delete Sessions and no session-deletion surface exists yet.
