...
若要以 settings 驱动组装（不写 `cordis.yml` 行），挂载 [`@deepseek-ai/dsh-mcp-servers`](../mcp-servers/README.zh.md)——base bundle 已这样挂载——并在 `$DSH_HOME/settings.yaml` 的 `mcp` 段声明服务器；它会应用同样的单实例配置，并让挂载集随已提交的编辑保持同步。
