...
视频走同一个对象存储，准入故事更轻：`saveVideo` 校验声明的容器魔数（MP4 `ftyp`、WebM/Matroska EBML、Ogg）与 `videoLimits` 字节上限，然后提交一个不带内在尺寸的 `VideoAttachmentRef`——存储自身没有解复用器，因此准入只证明容器良构，绝不证明可解码。`readVideo` 会重新校验摘要与容器。视频是单对象媒体（壁纸），因此没有批量校验成员：`saveVideo` 就是校验闸门。
