...
The per-session usage ledger: one durable row per session in its own [`storage-domain`](../../storage/storage-domain/README.md) domain `usage_ledger`, accumulated by a collector on the session event feed. Every usage-bearing `assistant/message` event adds its provider-reported buckets (input, output, cache read, cache write) to the session's row, bumps the sample count, and stamps the wall-clock time. Rendered by the web Usage settings section ([`dsh-client-ui-usage`](../../client/ui-usage/README.md)) through the generated `usageLedger` Remote namespace. The ledger is user-facing only — nothing here enters a session log, a model request, or any tool schema.
