---
description: "Durable per-session token-usage ledger accumulated from provider-reported samples."
...
Buckets mirror the provider sample vocabulary and accumulate per (turn, step); replay ordering makes double-counting a non-issue in legal logs.
