...
2026-08-27 自 `zhu1090093659/dsh-web` 的 `packages/dsh-git-graph` 收编入本
fork（Apache-2.0，保留上游 LICENSE）。相对上游的本地差异：tsdown 配置改为
从包内复制的 `./shared/` 引入共享构建助手（不再指向上游 monorepo 根），
`.npmrc` 为机器本地配置且已忽略。同步为手动：重新复制包目录与 shared 配置
后重放这些差异。
