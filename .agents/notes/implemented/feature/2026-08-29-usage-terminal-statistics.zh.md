...
- 域版本升级会一次性丢弃 v0 台账行：总计、按模型、按天的切片从升级后重新累计，而不是部分解读升级前的行。
- 模型与按天统计只覆盖升级之后记录的用量；总览条的总计保持完整，因为顶层桶保留了原形状。
- 费用报告只在部署显式配置了价格表时出现；未配置时渲染 token、请求数与缓存命中率，没有费用列。
- 聚合在 `view.ts` 中按客户端快照运行，宿主保持纯累加器；快照过期只会渲染过期统计，不产生额外 RPC 成本。