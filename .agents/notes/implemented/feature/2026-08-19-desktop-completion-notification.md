...
Users had no desktop-level awareness when a task completed if they were not watching the session tab. The existing sidebar's green `completed` reminder (the manager's `completedNotifications` edge) is presentation state owned by the list projection: it fires once per run, clears on selection, and suppresses the watched session entirely. It cannot serve as an OS-level notification mechanism, and "everything is a plugin" demands a dedicated feature package with its own edge detector and settings namespace.
