...
用户如果不盯着会话标签页，就无法在桌面级别感知任务何时完成。侧栏已有的绿色 `completed` 提醒（manager 的 `completedNotifications` 边沿）是列表投影拥有的呈现状态：每次运行只触发一次、选中即清除、且完全压制被盯着的会话。它无法充当 OS 级通知机制，而"一切皆插件"要求一个独立的特性包，自带边沿检测与设置命名空间。
