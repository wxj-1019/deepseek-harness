...
- **Route through the UI-selected session model instead of a deployment setting.** The user's requirement was a deployment-level setting changeable in Settings, not a per-session manual switch; the setting page is the product surface.
...
- Web e2e (`apps/web/tests/vision-route.e2e.ts`, keyless replay): the Settings page configures the vision model (aria golden), an image-bearing prompt routes to the vision model and the session stays there, recorded against the real DashScope qwen3-vl-plus route and replayed without keys.
