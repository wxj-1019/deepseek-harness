...
- **本机开关**——`$DSH_HOME/settings.yaml` 的 `dev-checks` 段（Web 设置页 → **开发校验**）：`e2e`、`coverage`、`snapshot`、`docSync`、`prePushTypecheck` 经 `scripts/dev-check-run.ts` 生效，`buildHygiene` 仅指导 agent 选择；全部默认开启，且在 `CI=true` 或显式全量运行时被忽略。
