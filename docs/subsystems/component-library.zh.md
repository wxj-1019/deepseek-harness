...
[English](component-library.md) | 中文

已学习组件库的存储域所有者：带评审状态的持久记录、按查询排序的匹配，以及画廊的快照/列表/汇总读取。每个持久写入——扫描、watch、模型工具、面板评审——都在域提交后广播 `component-library/changed`，面板据此重新拉取。

## 服务行为

[`ComponentLibraryService`](../../packages/storage/component-library/src/index.ts) 持有持久集合及其远程面；包 [README](../../packages/storage/component-library/README.zh.md) 与 [`types.ts`](../../packages/storage/component-library/src/types.ts) 定义可调用 API 和记录词汇。
