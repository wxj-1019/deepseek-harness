...
打开**设置 → 识图模型**可为部署指定一个支持图片的模型。配置后，消息携带图片的第一个回合会自动把请求路由到该模型——无需手动切换。会话随后停留在识图模型上：其历史已包含图片，纯文本模型之后无法再服务该会话（与模型选择器中"会话已包含图片"的拒绝规则相同）。

该页面只列出声明了图片输入模态的模型，因此纯文本的 DeepSeek 路由不会出现在其中。要让识图路由生效，须按图片输入一节配置识图模型——手工输入的模型需要 `input: [text, image]`（或路由的 `defaultInput`）。
